...
def generate_solid_field(size=18):
    ...
    # Фигура 1: Квадрат 4x4 (сдвинут левее)
    field[2:6, 1:5] = 1
...
def get_boundary_outline(binary):
    """Создает оболочку вокруг фигуры (внешние маркеры)"""
...
def count_corners(binary):
    """
    Считает количество углов d (блоки 2×2 с суммой 1 или 3 пикселя)
    """
    corners = 0
    ...
    for y in range(rows - 1):
        for x in range(cols - 1):
            ...
            if np.sum(block) in (1, 3):
                corners += 1
    return corners // 2  # Каждый угол учитывается дважды


def calculate_single_metric(binary):
    """Расчёт метрик согласно второму коду:
       P1 = b
       P2 = d * sqrt(2)
       P3 = b - 2*d + d*sqrt(2)
       K = P1² / S
...
# 3. ВИЗУАЛИЗАЦИЯ (СОГЛАСНО НОВЫМ ФОРМУЛАМ)
...
def visualize_solid(field):
    ...
    BLACK = [0.0, 0.0, 0.0]        # чёрные маркеры (для инверсии P2)

    # Общая маска границ (для всех фигур)
    total_boundary = np.zeros((h, w), dtype=bool)
    for m in metrics_list:
        total_boundary = total_boundary | m['boundary']

    # Утилита для быстрого создания изображения с заданным цветом маркеров
    ...
    # Панель 2: Инверсия P2 = d·√2 (чёрные маркеры)
    ax = axes[2]
    ax.set_aspect('equal')
    ax.set_xticks([])
    ax.set_yticks([])
    ...
    ax.text(0.98, 0.98, txt_P2, fontsize=9, va='top', ha='right',
            bbox=dict(boxstyle='round', facecolor='wheat', alpha=0.8))
    ax.set_title('3. Инверсия (d·√2)', fontsize=11, fontweight='bold')

    # Панель 3: Полный периметр P3 = b-2d+d√2 (серые маркеры)
    ax = axes[3]
    ax.set_aspect('equal')
    ax.set_xticks([])
    ax.set_yticks([])
    ...
    ax.text(0.98, 0.98, txt_P3, fontsize=9, va='top', ha='right',
            bbox=dict(boxstyle='round', facecolor='wheat', alpha=0.8))
    ax.set_title('4. Периметр (b-2d+d√2)', fontsize=11, fontweight='bold')

    # Панель 4: Коэффициент округлости K (текст, без изображения)
    ax = axes[4]
    ax.axis('off')
    ...
    print("\n" + "=" * 60)
    print("МЕТРИКИ ДЛЯ КАЖДОЙ ФИГУРЫ (по формулам из второго кода):")
    print("=" * 60)
    for j, m in enumerate(metrics_list, 1):
        print(f"\nФигура {j}:")
        print(f"  S (Площадь)        = {m['S']}")
        print(f"  P1 (b)             = {m['P1']}")
        print(f"  P2 (d·√2)          = {m['P2']:.2f}")
        print(f"  P3 (b-2d+d√2)      = {m['P3']:.2f}")
